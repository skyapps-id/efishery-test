...
func (r *pondFeedersRepositoryImpl) FatchPondFeeders(ctx context.Context, pondUuid string) ([]entity.PondFeeder, error) {
	...
	result := r.orm.WithContext(ctx).Select("pond_feeders.pond_uuid, pond_feeders.feeder_uuid, feeders.barcode").
		Joins("JOIN feeders feeders ON pond_feeders.feeder_uuid = feeders.uuid").
		Where("pond_feeders.pond_uuid = ?", pondUuid).Find(&pondFeeders)
